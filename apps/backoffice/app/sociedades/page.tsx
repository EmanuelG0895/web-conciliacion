...
export default function Sociedades() {
  ...
  const handleSave = (data: { rfc: string; razonSocial: string; numeroSociedadSAP: string }) => {
    if (editSociedad) {
      setSociedades((prev) =>
        prev.map((s) =>
          s.id === editSociedad.id
            ? { ...s, ...data }
            : s
        )
      );
    ...
    },
    {
      ...
      render: (sociedad: Sociedad) => (
        <div className="flex gap-2">
          <Button size="icon" variant="ghost" onClick={() => handleOpenEdit(sociedad)} title="Editar">
            <Edit2 className="w-4 h-4" />
          </Button>
          <Button size="icon" variant="ghost" onClick={() => setDeleteSociedad(sociedad)} title="Eliminar">
  ...
  return (
    <div className="flex flex-col gap-8 p-6 w-full">
      <div className="space-y-4">
        <div className="flex justify-between items-center">
          ...
          <Button onClick={handleOpenCreate} variant="default" size="sm">
            <Plus className="w-4 h-4 mr-1" /> Nueva Sociedad
          </Button>
        </div>
      ...
      <AlertDialogRoot open={!!deleteSociedad} onOpenChange={() => setDeleteSociedad(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
            ...
            <AlertDialogDescription>
              ¿Estás seguro de que deseas eliminar la sociedad <b>{deleteSociedad?.razonSocial}</b>? Esta acción no se puede deshacer.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
